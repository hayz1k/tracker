...
func (s *SiteStore) FindAll(ctx context.Context) ([]*site.Site, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&id, &domain, &consumerKey, &consumerSecret, &note, &merchant); err != nil {
			return nil, err
		}

		siteEntity, err := site.NewSite(id, domain, consumerKey, consumerSecret, note, merchant)
		if err != nil {
			return nil, err
		}
